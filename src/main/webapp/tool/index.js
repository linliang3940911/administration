...
// 图片集合
var imgs = [[12321324],[37647730928],[6],[8],[11],[37],[7645634403],[123],[1234],[435435],[1231231]]
/*for (var  i = 1; i < 17; i++) {
  imgs.push('' + i + i)
}*/
...
function fnDown(verX, verY) {
  swiper.mousedown(function() {
    ...
    box.bind('mouseup', function() {
      var stopL = verify.offset().left - 670
      // 误差在2px以内则算做成功
      if (Math.abs(stopL - verX)>50) {

          alert('验证失败')
      } else {

        alert('验证成功')
          location.href="/modle/queryHomePage"
      }
      // 解除绑定，并将滑动模块归位
      box.unbind('mousemove')
      swiper.css('left', '0px')
